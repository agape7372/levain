...
describe('§16 수용 기준 — 광고 0으로 전 변형 도달', () => {
  ...
  it('무료 경로만으로 변형 72종 전부 해금된다', () => {
    ...
    // 재료 30종 확장(2026-08-26)으로 72 → 154. 이 수치 자체가 아니라 **아래 루프가 상한 안에서
    ...
    let feeds = 0;
    // **실측 1181회**(재료 30종·변형 154종, 2026-08-26 재측정). 이전 판은 521회(12종·72종).
    ...
    // F(154)=1150인데 실측이 1181로 2.7% 위 — **가루 말고 다른 게 병목이 되지 않았다는 확인**이다
    // (5% 넘게 벗어나면 조사할 것).
    //
    // 하루 2회면 약 **591일**. 사용자 확정(2026-08-26): 상수는 안 건드리고 상한만 올린다 —
    // 가속은 과금이 판다(확장기획 §10 보상형 광고·IAP, 아직 미구현).
    // 느린 동거가 컨셉이라 길이 자체는 문제가 아니고, **닿는가**가 수용 기준이다.
    const MAX_FEEDS = 1536; // 1181 × 1.3
