...
describe('§16 수용 기준 — 광고 0으로 전 변형 도달', () => {
  ...
  it('무료 경로만으로 변형 160종 전부 해금된다', () => {
    const { store, clock } = matureStore();
    store.claimStarterGift('olive');
    // 통은 **주입하지 않는다.** 원가가 통으로 옮겨진 이상 그램도 무료 경로 위에 있고,
    // grantPantry로 채우면 이 수용 기준이 재는 대상을 우회해 버린다 — 아래 루프가
    // 실제 떼어내기(split)로만 벌어서 160종에 닿는지가 이 테스트의 전부다

    const targets = playableRules().map((r) => variantIdOf(r));
    // 재료 30종 확장(2026-08-26)으로 72 → 160. 이 수치 자체가 아니라 **아래 루프가 상한 안에서
    // 끝나는지**가 수용 기준이다 — 광고 0으로 전 변형에 도달 가능해야 한다
    expect(targets.length).toBe(160);
    const remaining = (): string[] => targets.filter((v) => store.getCollection()[v] === undefined);

    let feeds = 0;
    // **실측 1231회**(재료 30종·변형 160종, 2026-08-26 재측정). 이전 판은 521회(12종·72종).
    // 상한을 그냥 올리지 않고 **다시 쟀다** — 상한의 정의가 "실측의 약 1.3배"라서, 안 재고 올리면
    // 그 숫자가 무엇의 1.3배인지 아무도 모르게 된다.
    //
    // 사이클당 그램 예산(1:5:5 = 380g)이 그 주기에 굽는 양을 넘어서 그램은 병목이 아니고,
    // 병목은 여전히 재료(변형 첫 발견마다 1개)다. 닫힌 식 F(V) ≈ 10×(V−1−⌊V/5⌋−5)로 예측하면
    // F(160)=1200인데 실측이 1231로 2.6% 위 — **가루 말고 다른 게 병목이 되지 않았다는 확인**이다
    // (5% 넘게 벗어나면 조사할 것).
    //
    // 하루 2회면 약 **616일**. 사용자 확정(2026-08-26): 상수는 안 건드리고 상한만 올린다 —
    // 가속은 과금이 판다(확장기획 §10 보상형 광고·IAP, 아직 미구현).
    // 느린 동거가 컨셉이라 길이 자체는 문제가 아니고, **닿는가**가 수용 기준이다.
    const MAX_FEEDS = 1601; // 1231 × 1.3
