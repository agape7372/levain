 ...
 *
 * 도달 산술(수용 기준 "광고 0으로 전 변형 도달") — 재료 12종 확장(2026-08-25) 반영:
 * 변형 **72종** = 재료 72개, 온보딩 선물 1개를 빼면 71 × 12 = **852 가루**.
 * 일회성 = 성장 5단계 60 + 베이스 레시피 10종 120 = 180. 나머지는 급여·굽기 미션이 메운다.
 * 실측 완주 = 급여 521회(tests/economy.test.ts가 상한 MAX_FEEDS 700으로 강제).
 * 하루 2회면 ≈ 260일 — 광고는 가속만 판다.
 *
 * 보관 통(GDD §6-2)이 생긴 뒤에도 이 수치는 **그대로 521**이다. 사이클당 그램 예산이
 * 그 주기에 굽는 양보다 커서 병목이 아니고, 병목은 여전히 재료다 (같은 테스트가 재측정).
 */
