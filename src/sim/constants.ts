...
/**
 * 보관 통 (GDD §6-2) — 떼어낸 르방이 g으로 쌓이고 빵이 여기서 나간다.
 * SPLIT_MIN_G = 최저가 빵(플랫브레드 30g) 기준. 이보다 적으면 뗄 게 못 된다.
 * PANTRY_MAX는 밸런스 캡이 아니라 저장 가드다 — 도달 가능한 캡을 두면
 * "통이 꽉 차서 못 떼요"라는 막다른 상태와 그 문구가 생긴다.
 */
 ...
 *
 * 도달 산술(수용 기준 "광고 0으로 전 변형 도달") — 재료 **30종** 확장(2026-08-26) 반영:
 * 변형 **160종** = 재료 160개, 온보딩 선물 1개를 빼면 159 × 12 = **1908 가루**.
 * 일회성 = 성장 5단계 60 + 베이스 레시피 10종 120 = 180. 나머지는 급여·굽기 미션이 메운다.
 * 실측 완주 = 급여 **1231회**(tests/economy.test.ts가 상한 MAX_FEEDS 1601 = 실측×1.3으로 강제).
 * 하루 2회면 ≈ **616일**.
 *
 * ★**상수를 안 건드린 건 사용자 확정이다**(2026-08-26): 페이싱은 그대로 두고 상한만 재측정해 올린다,
 * 가속은 과금이 판다(확장기획 §10 보상형 광고·IAP — **아직 미구현**이라 지금은 파는 가속이 없다).
 * 지난 40→72 확장 때 밟은 선례와 같은 방식.
 *
 * 이전 판: 12종·72종·521회·≈260일. 보관 통(GDD §6-2) 도입은 이 수치를 안 바꿨다 —
 * 사이클당 그램 예산이 그 주기에 굽는 양보다 커서 그램은 병목이 아니고, 병목은 여전히 재료다.
 */
