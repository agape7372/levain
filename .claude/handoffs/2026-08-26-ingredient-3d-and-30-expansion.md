...
4. **신규 18종 이미지 — 본세션이 직접 생성 착수**(사용자 지시 "18종 이미지도 해줘").
   - higgsfield는 **크레딧 0·무료 할당 없음**으로 닫혔다(2크레딧/장 × 54장 = 108 필요).
   - Grok Bot 데스크탑 앱은 떠 있으나 GUI 자동화로 54장을 몰면 호출이 400건 넘고 취약하다
     (`코딩\grok-ops\tools\gui.ps1`, 호출당 1~2동작 제약).
   - **채택 = 사용자 로그인 Chrome으로 grok.com.** `grok.com/imagine`은 로그인 확인됐고
     비율 선택(1:1)·이미지 모드가 있다. 단 **Imagine은 단발 생성이라 ②③의 "같은 물체" 연속성이 없다** —
     프롬프트 시트가 전제한 건 스레드되는 채팅이다. 서브에이전트가 동작 방식을 확정 중.
   - ★**이미지는 최종 자산이 아니라 형태·비율 레퍼런스**다(색 정본은 프롬프트 JSON hex) —
     생성기가 그록이 아니어도 되는 근거. 다만 세 뷰가 **서로 같은 물체**인 건 양보 불가
     (비율을 뷰 간에 재기 때문).
5. **신규 18종 GLB** — 이미지가 들어온 뒤. 파이프라인은 이미 12종으로 검증됐다.
