...
5. ★**신규 18종 GLB 완성 — 재료 30/30 전량 3D 확보.** 합계 1038KB / 1920KB.
   ...
   - **64px 판독: 14 통과 / 2 실패 → 둘 다 고쳤고 재판정 대기.**
     `sunflowerseed`(색 — 낱알별 버킷 배정 뒤집기, tri 0 증가) ·
     `poppyseed`(실루엣 — 더미:낱알 비율 0.12~0.22배 → 0.38~0.58배, tri 0 증가).
     ★수정은 들어갔고 재베이크·roundtrip까지 확인했지만 **재판정 결과는 아직 못 받았다.**
     ★검사가 결정적 A/B를 짚었다: `flaxseed`와 `sunflowerseed`가 **거의 같은 "1+n 겹침" 구성인데
     승패가 갈렸고 차이는 낱알 간 명도 대비뿐**이었다 — 원인 추측을 건너뛸 수 있었다.
