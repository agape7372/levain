...
2. **앱 배선은 코드로 확인 완료** — 김 게이트(`app.ts:253` `if (kind === 'bread')`) ·
   GLB 경로 분기(`:241`) · 재료 카드 `button`(`recipes.ts:424`) · 썸네일 폴백(`:341`·`:345`).
   `npm run build` green, `dist/ingredients/`에 GLB 12 + 썸네일 12가 실렸다(**번들 +764KB**,
   30종이면 약 1.9MB). **런타임 실동작**(탭 → 3D가 실제로 뜨는지, 토스트, 씬 복원, 콘솔 에러)은 미확인.
