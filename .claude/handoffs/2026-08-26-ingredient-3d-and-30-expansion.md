...
1. **도감 실물 검증** — QA 에이전트가 캡처는 했으나 **보고를 못 받았다.**
   확인할 것: 64px 판독 12종 / 카드 탭 → 3D / **김이 안 나오는지** / 미발견 탭 토스트 /
   뒤로가기 후 홈 씬 복원 / 콘솔 에러 / 교환소·선물 모달이 30종에서 스크롤되는지.
2. **`rosemary` 108tri · `cinnamon` 206tri**가 세장군 목표(400~900) 하한보다 낮다.
   64px 판독 단순화 결과인지 미확인 — 다른 10종은 전부 목표 범위 안이라 이 둘만 튄다.
   의도된 단순화면 크립에 "정답"이라고 근거를 남겨야 다음 사람이 안 되돌린다.
3. **신규 18종 GLB** — 사용자가 그록 이미지를 넣은 뒤. 이번 범위 밖.
4. **실기기 확인** — 1.3.0·1.3.1·1.3.2 세 발행이 여전히 미확인으로 쌓여 있다(이번 세션과 무관).
