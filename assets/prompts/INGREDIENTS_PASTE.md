...
- 광택·하이라이트 없는 matte인가 — **채도는 높아도 되지만 광택은 안 된다**
  - ⚠**단, 광택 하나로 폐기하지는 마라.** 원래 여기 적혀 있던 "알베도에 흰 얼룩으로 구워진다"는
    **image-to-3D 시절 사유**다(2026-08-24에 three.js 절차 모델링으로 바뀌었다). 지금 빌더는
    이미지를 텍스처로 안 쓴다 — **색 정본은 프롬프트 JSON hex, 이미지는 형태·비율만**(`docs/BREADS.md:43`).
    광택이 **형태를 가려 못 읽을 때만** 폐기한다.
